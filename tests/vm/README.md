# VM test harness (kernel 4.15 / 5.10, 3-machine source-IP validation)
...
then with the proxy intercepting. Select the kernel B (the router/proxy) runs
with via `--kernel {4.15,5.10}` (default `4.15`).

```
A (client, 10.10.1.10) ──br-left── B (router/proxy, kernel 4.15|5.10) ──br-right── C (HTTPS, 10.10.2.10)
...
| 5.10 (lvh `5.10.260`)       | **PASS** | **SKIP** — test-kernel limit (see below) |

The **eBPF** plane — what the product ships by default — is validated end-to-end
on both kernels. On 5.10 it takes the `clsact+tc` path (TCX needs ≥ 6.6); the four
`TCX attach unavailable` lines are logged at DEBUG and are expected, not a fault.

The **iproute** plane passes on 4.15 and is **skipped on the lvh 5.10 kernel**: that
lean BPF-testing kernel is built without `NETFILTER_XT_MATCH` (no `xt_tcpudp`), so
the iptables `-p tcp --dport` match can't load. The harness detects this and skips
with a message — it is a limitation of the *test kernel*, not the proxy. A real
distro 5.10 kernel (e.g. `5.10.0-*-generic`/`-amd64`) ships `xt_tcpudp`, so the
iproute plane works there just as it does on the 4.15 full kernel.
...
The first `up` downloads the Ubuntu cloud image(s) into `tests/vm/.work/images/`
(jammy always; bionic for `--kernel 4.15`) and, for `--kernel 5.10`, the lvh
kernel into `tests/vm/.work/lvh/`; all are cached and reused. Everything the
harness writes lives under `tests/vm/.work/` (git-ignored).
